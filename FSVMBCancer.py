...
class BCancerPrediction:

    def __init__(self, data = "diabetes"):
        self.data = data

    def data_processing(self, fileName='breast_cancer_heart_disease_data.csv'):
        ...
        #print(len(X))
        #print(len(ytrue))
        #print(len(ytrue[ytrue==0]))
        #print(len(ytrue[ytrue == 1]))
        ytrue = np.array(ytrue, dtype=np.float)
        return X, ytrue, sc_X
    ...
    def process(self):
        ...
        y_pred_prob_svm = self.model2.predict_proba(X_test)[:, 1]
...
# plot model decision boundary
#fig, ax = plt.subplots()
#D.plot_boundary(plt, D.ssmodel, 'Fast SVM Breast Cancer decision boundary')
#D.plot_boundary(plt, D.TSVMmodel, 'S3VM Breast Cancer decision boundary')
D.plot_boundary(plt, D.S3VMmodel, 'CPLE Breast Cancer decision boundary')
